...
void MazeWidget::callStudentsSearch(bool recursive)
{
    ...
    bool found = recursive ? ::didFillPathToGoal(&_mazeModel, rk._r, rk._k)
                           : ::didFillPathToGoalFIFO(&_mazeModel, rk._r, rk._k);
    if (!found)
...
QRectF MazeWidget::rcFromRK(RK rk){
    return QRectF(rk._k*_widthOfCell, rk._r*_heightOfCell, _widthOfCell, _heightOfCell);
}
...
void MazeWidget::paintEvent(QPaintEvent *event)
{
    ...
    for (int r = rkTopLeft._r ; r<=rkBottRight._r ; r+=1)
        for (int k = rkTopLeft._k ; k<=rkBottRight._k ; k+=1){
...
void MazeWidget::showContextMenu(const QPoint &point){
    ...
    if (_mazeModel.isInside(rk._r, rk._k) && !_mazeModel.isWall(rk._r, rk._k)){
        QMenu menu;
